...
class BudgetController extends AbstractController
{
    ...
    public function index(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if (!empty($wallets)) {
            ...
            foreach ($budgets as $budget) {
                ...
                // ✅ Fix line 98 — use createFromInterface
        ...
        $limit            = 6;
        $totalActivePages = max(1, (int) ceil(count($activeBudgets) / $limit));
        if ($activePage < 1) $activePage = 1;
        if ($activePage > $totalActivePages) $activePage = $totalActivePages;

        // ✅ Fix line 134 — cast to int
        $paginatedActive = array_slice($activeBudgets, (int)(($activePage - 1) * $limit), $limit);

        $expiredPage       = $request->query->getInt('expired_page', 1);
        $totalExpiredPages = max(1, (int) ceil(count($expiredBudgets) / $limit));
        if ($expiredPage < 1) $expiredPage = 1;
        if ($expiredPage > $totalExpiredPages) $expiredPage = $totalExpiredPages;

        // ✅ Fix line 141 — cast to int
        $paginatedExpired = array_slice($expiredBudgets, (int)(($expiredPage - 1) * $limit), $limit);
    ...
    public function step3(
        ...
        // ✅ Fix lines 251-252 — check wallet is not null
        ...
        if ($request->isMethod('POST')) {
            ...
            // ✅ Fix line 237 — cast to string
            $date = $request->request->get('dateBudget');
            $budget->setDateBudget($date ? new \DateTime((string) $date) : null);
            ...
            if ($budget->getMontantMax() > $wallet->getSolde()) {
                $this->addFlash('error',
                    'Budget amount (' . $budget->getMontantMax() . ' ' . $wallet->getDevise() .
                    ') exceeds your wallet balance (' . $wallet->getSolde() . ' ' . $wallet->getDevise() . ')!'
                );
                return $this->render('management/budget/step3.html.twig', [
                    'wallet'    => $wallet,
                    'categorie' => $categorie,
    ...
    public function delete(
        ...
        // ✅ Fix line 309 — cast to string
        if ($this->isCsrfTokenValid('delete' . $budget->getId(), (string) $request->request->get('_token'))) {
